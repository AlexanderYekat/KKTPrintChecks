...
//function GetCommState1(hFile: THandle; lpDCB: DWord): Bool; stdcall; external 'kernel32.dll' name 'GetCommState';
//function SetCommState(hFile: THandle; lpDCB: _DCB): Bool; overload; external 'kernel32.dll';
...
//    FDCB1: TApiDCB;
  ...
  public
    ...
    function PutBlock(const Buf; Count: Integer): Integer;
    { Put data in COM-port. Check: Count <= OutBuffFree }

    function GetBlock(var Buf; Count: Integer): Integer;
    ...
    property InSize: integer read IBuffSize write IBuffSize;
    { Input buffer size }

    property OutSize: integer read OBuffSize write OBuffSize;
    { Output buffer size }
  ...
  Repeat
    if (FComPort.OBuffUsed > 0) then begin
      ...
      if FComPort.Enabled and(BytesToWrite>0)and
         (not FComPort.FKillThreads) {and FComPort.CTS} then begin
...
begin
  ...
  Repeat
    if WaitForSingleObject(FcomPort.ReadReadyEvent, 50) = WAIT_OBJECT_0 then begin
      if FComPort.FKillThreads then Break;
...
begin
  ...
  repeat
    ...
    if FComPort.FKillThreads then Break;
    if OK and ((FStatus and EV_RXCHAR) > 0) then begin
...
//  GetMem(FDCB1,SizeOf(FDCB1^));
//  FDCB1^.DCBLength:=SizeOf(FDCB1^);
end;
...
//    CommTimeouts.ReadIntervalTimeout := MAXDWORD;
    ...
    WaitForSingleObject(sEvent, 1000);
    ...
    WaitForSingleObject(rEvent, 1000);
    WriteThread := TWriteThread.Create( Self );
    WaitForSingleObject(wEvent, 1000);
    FOpen := True;
...
function TCustomCommPort.PutBlock(const Buf; Count: Integer): Integer;
begin
  Result := 0;
...
function TCustomCommPort.GetBlock(var Buf; Count: Integer): Integer;
begin
  Result := 0;
  if not Enabled then exit;
  EnterCriticalSection(ReadSection);
  ...
  Move(IBuffer[Result], IBuffer[0], IBuffUsed-Result);
...
begin
  ...
  if Enabled then
  begin
//    BuildCommDCB('com1 baud=9600 data=8 parity=N stop=1 octs=off odsr=off idsr=off rts=off dtr=off', FDCB);
    GetCommState(FComHandle, FDCB);
    FDCB.BaudRate := CBR[FBaudRate];
    FDCB.Flags    := 0; //AllHdwFlow; {????}
//    FDCB.Flags := (FDCB.Flags and not $30) or (DTR_CONTROL_ENABLE shl 4);
...
begin
  ...
  if FOpen then begin
    GetLastError;
    if FDTR then
      res := EscapeCommFunction(FComHandle, Windows.SETDTR)
    else res := EscapeCommFunction(FComHandle, Windows.CLRDTR);
    {$IFDEF DEBUG}
